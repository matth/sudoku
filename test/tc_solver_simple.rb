...
class TestSolverSimple < Test::Unit::TestCase
	...
	def test_valid_puzzles
		...
		puzzles.each{|puzzle|			
			grid = Sudoko::Grid.new(puzzle[:puzzle])
			solver = Sudoko::Solver::Simple.new(grid)
	...
	def test_not_enough_givens
		...
		assert_raise(Exception) {Sudoko::Solver::Simple.new(grid)}
	end
	...
	def test_unsolvable_square
		grid = Sudoko::Grid.new("..9.287..8.6..4..5..3.....46.........2.71345.........23.....5..9..4..8.7..125.3..")	
		solver = Sudoko::Solver::Simple.new(grid)
		assert_equal(0, solver.solutions.length)	
	end

	def test_unsolvable_box
		grid = Sudoko::Grid.new(".9.3....1....8..46......8..4.5.6..3...32756...6..1.9.4..1......58..2....2....7.6.")	
		solver = Sudoko::Solver::Simple.new(grid)
		assert_equal(0, solver.solutions.length)	
	end
	
	def test_unsolvable_col
		grid = Sudoko::Grid.new("....41....6.....2...2......32.6.........5..417.......2......23..48......5.1..2...")	
		solver = Sudoko::Solver::Simple.new(grid)
		assert_equal(0, solver.solutions.length)	
	end	
	
	def test_unsolvable_row
		grid = Sudoko::Grid.new("9..1....4.14.3.8....3....9....7.8..18....3..........3..21....7...9.4.5..5...16..3")	
		solver = Sudoko::Solver::Simple.new(grid)
		assert_equal(0, solver.solutions.length)	
	end	
	
	def test_not_unique_two_solutions
		grid = Sudoko::Grid.new(".39...12....9.7...8..4.1..6.42...79...........91...54.5..1.9..3...8.5....14...87.")
		solver = Sudoko::Solver::Simple.new(grid)
		assert_equal(2, solver.solutions.length)	
	end

	def test_not_unique_three_solutions
		grid = Sudoko::Grid.new("..3.....6...98..2.9426..7..45...6............1.9.5.47.....25.4.6...785...........")
		solver = Sudoko::Solver::Simple.new(grid)
		assert_equal(3, solver.solutions.length)	
	end
	
	def test_not_unique_four_solutions
		grid = Sudoko::Grid.new("....9....6..4.7..8.4.812.3.7.......5..4...9..5..371..4.5..6..4.2.17.85.9.........")
		solver = Sudoko::Solver::Simple.new(grid)
		assert_equal(4, solver.solutions.length)	
	end	

	def test_not_unique_ten_solutions
		grid = Sudoko::Grid.new("59.....486.8...3.7...2.1.......4.....753.698.....9.......8.3...2.6...7.934.....65")
		solver = Sudoko::Solver::Simple.new(grid)
		assert_equal(10, solver.solutions.length)	
	end	

	def test_not_unique_one_hundred_and_twenty_five__solutions
		grid = Sudoko::Grid.new("...3165..8..5..1...1.89724.9.1.85.2....9.1....4.263..1.5.....1.1..4.9..2..61.8...")
		solver = Sudoko::Solver::Simple.new(grid)
